...
export class BotService {
  ...
  // CRITICAL BUSINESS RULE (2026-04): The bot must NEVER answer with fixed, prewritten text.
  // Every outbound message must be generated by the AI engine using the dynamic modules:
  // PRODUCTOS + INSTRUCCIONES + EMPRESA.
  private isAiAlwaysOn(): boolean {
    return true;
  }
  ...
  private async generateValidatedReply(params: {
    ...
    const mediaRequestedButUnavailable =
      params.companyCheck.reason === 'require_catalog_media' && params.candidateMediaFiles.length === 0;
    const effectiveCompanyCheck: CompanyRuleCheck = mediaRequestedButUnavailable
      ? {
          ...params.companyCheck,
          allowResponse: true,
          // Avoid hard rejecting on media count when there is no media available.
          reason: 'require_catalog_media_unavailable',
        }
      : params.companyCheck;
    ...
    for (let attempt = 0; attempt <= BotService.MAX_REPLY_REGENERATION_ATTEMPTS; attempt += 1) {
      const companyRuleInstruction = buildCompanyRuleInstruction(
        params.message,
        params.companyData,
        effectiveCompanyCheck,
      );
      const mergedContext = [
        companyRuleInstruction,
        mediaRequestedButUnavailable
          ? '[MEDIA] El cliente pidio fotos/videos, pero ahora mismo NO hay media disponible para enviar. Responde con texto, sin inventar, y ofrece avisar cuando haya fotos reales.'
          : '',
        params.context,
      ...
      if (!this.isAiAlwaysOn() && candidateFallback) {
        console.log('RESPUESTA FINAL:', {
    ...
    // Last resort: still respond via IA (never send fixed text).
    const emergency = await this.aiService.generateReply({
      config: params.config,
      fullPrompt: params.fullPrompt,
      companyContext: params.companyContext,
      contactId: params.contactId,
      message: params.message,
      history: params.history,
      context: this.limitText(
        [
          params.context,
          buildConversationMemoryContext(params.conversationMemory),
          lastReason
            ? this.buildRetryInstruction(params.conversationMemory, lastReason, lastRejectedText)
            : 'Responde diferente, con producto + valor minimo, sin sonar genérico.',
        ].filter(Boolean).join('\n\n'),
  ...
  private buildAiAuditModuleStats(params: {
    ...
    // New rule: PRODUCTOS is mandatory for EVERY response.
    // Even company-only questions must still be answered with at least one product + value.
    // If there is no catalog configured, the bot must block instead of answering.
    const requireProductos = true;

    const missing: Array<'INSTRUCCIONES' | 'PRODUCTOS' | 'EMPRESA'> = [];
    if (!hasInstrucciones) {
      missing.push('INSTRUCCIONES');
    }
    if (requireProductos && (!hasProductos || (params.allProducts ?? []).length === 0)) {
  ...
  private async buildFallbackResult(
    ...
    let botConfig: Awaited<ReturnType<BotConfigService['getConfig']>> | null = null;
    try {
      botConfig = await this.botConfigService.getConfig();
    } catch {
      botConfig = null;
    ...
    const intent = this.detectIntent(message);
    const decision = this.buildFallbackDecisionState(contactId, message);

    // Emergency path: attempt IA first.
    let replySource: BotReplyResult['source'] = 'fallback';
    let fallbackReply = '';
    if (config && botConfig) {
      try {
        ...
        const emergency = await this.withTimeout(
          this.aiService.generateReply({
            ...
            context: [
              '[MODO_EMERGENCIA]',
              'Hubo un error interno en el flujo. Responde igual de forma humana, breve y útil.',
              'No inventes datos. Usa PRODUCTOS/EMPRESA del contexto. Evita repetir respuestas anteriores.',
            ].join('\n'),
            classifiedIntent: decision.intent,
            decisionAction: decision.action,
            purchaseIntentScore: decision.purchaseIntentScore,
          ...
          Math.min(6000, BotService.AI_TIMEOUT_MS),
          'ai_emergency',
        );

        if (emergency.content.trim()) {
          fallbackReply = emergency.content.trim();
          replySource = 'ai';
        }
      } catch {
